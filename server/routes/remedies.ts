...
router.get('/', optionalAuth, async (req: AuthRequest, res) => {
  try {
    ...
    if (search) {
      conditions.push(`(hr.title ILIKE $${paramIndex} OR hr.description ILIKE $${paramIndex})`);
      params.push(`%${search}%`);
      paramIndex++;
    }
    ...
    const remedies = result.rows.map(row => ({
      ...row,
...
router.get('/:remedyId', optionalAuth, async (req: AuthRequest, res) => {
  try {
    ...
    const remedy = {
      ...
      avg_rating: remedyResult.rows[0].avg_rating ? parseFloat(remedyResult.rows[0].avg_rating) : 0
    };
...
// Rate a remedy
router.post('/:remedyId/rate', authenticateToken, async (req: AuthRequest, res) => {
  try {
    const { remedyId } = req.params;
    ...
    const remedyCheck = await pool.query('SELECT id FROM home_remedies WHERE id = $1', [remedyId]);
...
router.put('/:remedyId', authenticateToken, async (req: AuthRequest, res) => {
  try {
    const { remedyId } = req.params;
    const { title, description, ingredients, instructions, category } = req.body;
    const user_id = req.user!.id;
    ...
    const result = await pool.query(
      `UPDATE home_remedies 
       ...
       RETURNING *`,
      [title, description, ingredients, instructions, category, remedyId, user_id]
    );

    if (result.rows.length === 0) {
      return res.status(404).json({ error: 'Remedy not found or not authorized to update' });
    }
...
router.delete('/:remedyId', authenticateToken, async (req: AuthRequest, res) => {
  try {
    const { remedyId } = req.params;
    const user_id = req.user!.id;
    ...
    if (result.rows.length === 0) {
      return res.status(404).json({ error: 'Remedy not found or not authorized to delete' });
    }
