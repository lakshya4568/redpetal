...
router.post('/posts', authenticateToken, async (req: AuthRequest, res) => {
  try {
    const { title, content, category = 'general', is_anonymous = false, images = [] } = req.body;
...
router.get('/posts', optionalAuth, async (req: AuthRequest, res) => {
  try {
    ...
    const posts = result.rows.map(row => ({
      ...
      images: row.images ? row.images.filter(img => img !== null) : []
    }));
...
router.get('/posts/:postId', optionalAuth, async (req: AuthRequest, res) => {
  try {
    ...
    const post = {
      ...
      images: postResult.rows[0].images ? postResult.rows[0].images.filter(img => img !== null) : []
    };
...
// Like/unlike a post
router.post('/posts/:postId/like', authenticateToken, async (req: AuthRequest, res) => {
  try {
    const { postId } = req.params;
    const user_id = req.user!.id;
...
router.get('/posts/:postId/comments', optionalAuth, async (req: AuthRequest, res) => {
  try {
    const { postId } = req.params;
...
router.delete('/posts/:postId', authenticateToken, async (req: AuthRequest, res) => {
  try {
    const { postId } = req.params;
    const user_id = req.user!.id;
