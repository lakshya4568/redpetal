  ...
  asyncHandler(async (req: AuthRequest, res) => {
    try {
      ...
      const avgCycleLength = Math.round(
        cycles.reduce(
          (sum: number, cycle: any) => sum + cycle.cycle_length,
          0
        ) / cycles.length
      );
      ...
      const avgPeriodLength = Math.round(
        cycles.reduce(
          (sum: number, cycle: any) => sum + (cycle.period_length || 5),
          0
        ) / cycles.length
      );
  ...
  asyncHandler(async (req: AuthRequest, res) => {
    try {
      ...
      if (!date || !symptom_type || !severity) {
        return res
          .status(400)
          .json({ error: "Date, symptom type, and severity are required" });
      }
  ...
  asyncHandler(async (req: AuthRequest, res) => {
    try {
      ...
      if (!date || !mood_type || !intensity) {
        return res
          .status(400)
          .json({ error: "Date, mood type, and intensity are required" });
      }
