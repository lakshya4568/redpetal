  ...
  asyncHandler(async (req, res) => {
    ...
    `);
      ...
      const typesWithCounts = defaultTypes.map((type) => {
        const found = typesResult.rows.find(
          (row: any) => row.resource_type === type
        );
        return {
          type: type,
          count: found ? parseInt(found.count) : 0,
  ...
  asyncHandler(async (req: AuthRequest, res) => {
    try {
      ...
      if (result.rows.length === 0) {
        return res
          .status(404)
          .json({ error: "Resource not found or not authorized to update" });
      }
  ...
  asyncHandler(async (req: AuthRequest, res) => {
    try {
      ...
      if (result.rows.length === 0) {
        return res
          .status(404)
          .json({ error: "Resource not found or not authorized to delete" });
      }
