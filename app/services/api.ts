...
// Resolve API base URL that works on simulators, emulators, and physical devices
const resolveApiBaseUrl = (): string => {
  // 1) Prefer explicit env var (Expo auto-injects EXPO_PUBLIC_* at build time)
  ...
  // 2) Try to infer the host from Expo dev server
  // Different SDKs expose this differently; try a few safely
  const anyConst: any = Constants as any;
  const hostUri: string | undefined =
    anyConst?.manifest?.debuggerHost ||
    anyConst?.expoConfig?.hostUri ||
    anyConst?.manifest2?.extra?.expoGo?.developer?.host ||
    anyConst?.manifest2?.extra?.expoClient?.host;

  let host = "localhost";
  if (hostUri && typeof hostUri === "string") {
    // e.g. "192.168.1.10:8081" -> "192.168.1.10"
  ...
  // 3) Default to port 3000 for local server
